...
```bash
docker run -p [LOCAL_PORT]:[EXPOSED_PORT] [IMAGE_ID] # -p publishes the port and routes the docker container port to our host machine's port otherwise it's a locked network container and it won't be reachable from outside. LOCAL_PORT is the published port that your application will be available to view on.
```
